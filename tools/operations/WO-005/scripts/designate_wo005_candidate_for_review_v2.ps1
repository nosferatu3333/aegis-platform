...
$sourceBranch = "documentation/wo-005-environment-interaction-specification"
...
Write-Host "`n=== LOCATE SOURCE DELIVERABLE COMMIT ==="
...
try {
    git -C $repo show-ref --verify --quiet $sourceRef
    $sourceRefExit = $LASTEXITCODE
}
finally {
    $ErrorActionPreference = $savedPreference
}

if ($sourceRefExit -eq 0) {
    $sourceCommit = (
        git -C $repo rev-parse $sourceRef
    ).Trim()

    Write-Host "Source branch found: $sourceBranch"
}
elseif ($sourceRefExit -eq 1) {
    $matchingCommits = @(
        git -C $repo log `
            --all `
            --format="%H`t%P`t%s" `
            --grep="^$([regex]::Escape($sourceSubject))$"
    )

    if ($LASTEXITCODE -ne 0) {
        throw "Unable to search Git history for the WO-005 deliverable."
    }

    $matchingCommits = @(
        $matchingCommits |
            Where-Object {
                ($_ -split "`t", 3)[2] -eq $sourceSubject
            }
    )

    if ($matchingCommits.Count -eq 1) {
        $sourceCommit = (
            $matchingCommits[0] -split "`t", 3
        )[0].Trim()

        Write-Host "Source commit found by exact subject."
    }
    elseif ($matchingCommits.Count -eq 0) {
        throw "No local WO-005 deliverable commit was found. Run the deliverable creation script first."
    }
    else {
        Write-Host "Matching commits:"
        $matchingCommits | ForEach-Object { Write-Host " - $_" }
        throw "More than one WO-005 deliverable commit matches the exact subject."
    }
}
else {
    throw "Unable to inspect the WO-005 source branch."
}

$sourceParent = (
    git -C $repo rev-parse "${sourceCommit}^"
).Trim()

$sourceTree = (
    git -C $repo rev-parse "${sourceCommit}^{tree}"
).Trim()

$sourceCommitSubject = (
    git -C $repo log -1 --format=%s $sourceCommit
).Trim()
...
try {
    ...
    try {
        git -C $canonicalWorktree cherry-pick --abort 2>$null | Out-Null
    }
    finally {
        $ErrorActionPreference = $savedPreference
    }
...
foreach ($path in $deliverablePaths) {
    $sourceBlob = (
        git -C $repo rev-parse "${sourceCommit}:$path"
    ).Trim()
