...
$expectedMain = "be7502f73b51808d54728f912ead46ad0073c7b9"
...
$trackingMain = (git -C $repo rev-parse refs/remotes/origin/main).Trim()

$remoteLines = Invoke-GitRead {
...
$remoteMain = (($remoteLines[0] -split "\s+")[0]).Trim()

if (
    $localMain -ne $expectedMain -or
    $trackingMain -ne $expectedMain -or
    $remoteMain -ne $expectedMain
) {
    throw "Local, tracking, or live remote main does not match the WO-004 closure."
}

[pscustomobject]@{
    LocalMain  = $localMain
    OriginMain = $trackingMain
    LiveMain   = $remoteMain
    Alignment  = "PASS"
} | Format-List
...
$workOrderPaths = @(
    Invoke-GitRead {
        git -C $repo ls-tree -r --name-only main governance/work-orders
...
$futureEvidence = @(
    Invoke-GitRead {
        git -C $repo grep -n -I -i -E $futureExpression main -- `
...
$areaSummary = foreach ($area in $areas) {
    $matches = @(
        Invoke-GitRead {
            git -C $repo grep -n -I -i $area main -- aegis_os docs
...
$trackingMainAfter = (git -C $repo rev-parse refs/remotes/origin/main).Trim()
$remoteLinesAfter = Invoke-GitRead {
    git -C $repo ls-remote --heads origin refs/heads/main
} "Final live remote main lookup"
$remoteMainAfter = (($remoteLinesAfter[0] -split "\s+")[0]).Trim()

if ($localMainAfter -ne $localMain) {
    throw "Local main changed during the inventory."
}

if ($trackingMainAfter -ne $trackingMain) {
    throw "origin/main changed during the inventory."
}

if ($remoteMainAfter -ne $remoteMain) {
    throw "Live remote main changed during the inventory."
}
