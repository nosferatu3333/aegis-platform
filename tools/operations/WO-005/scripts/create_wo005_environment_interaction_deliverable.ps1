...
$worktree = "$env:USERPROFILE\Projects\aegis-platform-wo-005"
$branch = "documentation/wo-005-environment-interaction-specification"

$base = "7a34c38d6210a2ed58f8966b3143ab67103424e4"
...
if ($localMain -ne $base) {
    throw "Local main does not match the authorized WO-005 base."
}
...
- Authoritative base: `7a34c38d6210a2ed58f8966b3143ab67103424e4`
...
'@
    ...
    if ($finalRemote -ne $base) {
        throw "Live remote main changed during deliverable preparation."
    }

    if ($finalTracking -ne $base) {
        throw "origin/main changed during deliverable preparation."
    }

    if ($finalLocalMain -ne $base) {
        throw "Local main changed during deliverable preparation."
    }

    Assert-AllWorktreesClean $repo

    Write-Host "`n=== WO-005 DELIVERABLE RESULT ==="

    [pscustomobject]@{
        AuthoritativeBase = $base
        DeliverableCommit = $deliverableCommit
        DeliverableParent = $deliverableParent
        DeliverableTree = $deliverableTree
        DeliverableSubject = $deliverableSubject
        DeliverablePathCount = $committedPaths.Count
        ADR006Status = "ACCEPTED IN DELIVERABLE"
        ArchitectureStatus = "ACCEPTED IN DELIVERABLE"
        ...
        LiveRemoteMainUnchanged = $true
        RemoteMutation = "NONE"
        WorktreesClean = $true
        CandidateDesignation = "NOT YET GRANTED"
        IntegrationAuthorized = $false
        PublicationAuthorized = $false
        RuntimeImplementationAuthorized = $false
        Verdict = "WO-005 LOCAL DELIVERABLE READY FOR DESIGNATION"
    } | Format-List

    Write-Host "Deliverable paths:"
    $committedPaths | ForEach-Object { Write-Host " - $_" }

    Write-Host "`nWO-005 LOCAL DELIVERABLE: COMPLETE"
    Write-Host "The four-document deliverable is committed locally and ready for designation."
    Write-Host "No push, main update, runtime implementation, tag, release, or cleanup was performed."
}
catch {
    if (Test-Path -LiteralPath $worktree) {
